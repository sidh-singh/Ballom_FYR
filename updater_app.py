"""
updater_app.py — SHA Signal Analysis updater service (NIFTY ONLY).

Branch: dev_update_nifty
Usage:  python updater_app.py [demo|live]

This branch handles ONLY the NIFTY index symbol.
Sibling branches:
...
Forever Loop (runs 24x7):
  Step 1 — Load Fyers token from C:/Ballom_FYR/fyers_token.json
           (prefer scanner's token; fallback to full self-auth if expired)
  Step 2 — Day-change detection -> reload token + holidays
  Step 3 — Read CE/PE pairs from option_pairs.json
           (generated by dev_scanner), filter to NIFTY only
  Step 4 — For the NIFTY pair, compute:
...
Token sharing:  prefers token from C:/Ballom_FYR/fyers_token.json
                (written by dev_scanner), falls back to self-auth
                via TOTP if token is missing, expired, or invalid
"""
...
ACTIVE_MARKET  = "INDEX"   # INDEX or COMMODITY
...
def main():
    ...
    # ── Load token (prefer scanner's, fallback self-auth) ──────────────────
    ...
    while True:
        ...
        # ── Step 3: Determine active market window (NIFTY = INDEX only) ──
        in_idx_window = INDICES_START <= current_time <= INDICES_END
        in_com_window = False   # NIFTY branch — no commodity processing

        if not in_idx_window:
            write_app_status(
                mode, str(current_day), status="idle",
                message=f"Outside NIFTY market hours ({now.strftime('%H:%M')})",
            )
            sleep(UPDATE_INTERVAL)
            continue

        # ── Step 4: Load INDEX pairs, filter to ACTIVE_SYMBOLS only ────────
        symbols_processed = 0
        symbols_failed = 0
