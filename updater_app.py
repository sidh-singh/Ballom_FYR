"""
updater_app.py — SHA Signal Analysis updater service (GOLDM ONLY).

Branch: dev_update_gold
Usage:  python updater_app.py [demo|live]

This branch handles ONLY the GOLDM commodity symbol.
Sibling branches:
  dev_update_nifty  -> NIFTY
  dev_update_silver -> SILVERM
...
Forever Loop (runs 24x7):
  ...
  Step 3 — Read CE/PE pairs from commodity_pairs.json
           (generated by dev_scanner), filter to GOLDM only
  Step 4 — For the GOLDM pair, compute:
...
# ── Symbol filter — this branch ONLY processes GOLDM ───────────────────────────
...
def main():
    ...
    while True:
        ...
        # ── Step 3: Determine active market window (GOLDM = COMMODITY only) ───
        in_idx_window = False   # GOLDM branch — no index processing
        in_com_window = COMMODITY_START <= current_time <= COMMODITY_END

        if not in_com_window:
            write_app_status(
                mode, str(current_day), status="idle",
                message=f"Outside GOLDM market hours ({now.strftime('%H:%M')})",
            )
        ...
        write_app_status(
            ...
            message=(
                f"[GOLDM] Updated {symbols_processed} symbol(s) "
