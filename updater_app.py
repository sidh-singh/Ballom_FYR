...
Forever Loop (runs 24x7):
  Step 1 — Load Fyers token from C:/Ballom_FYR/fyers_token.json
           (written by dev_scanner — prefer scanner's token; fallback
            to full self-auth if expired)
...
Token sharing:  reads token from C:/Ballom_FYR/fyers_token.json
                (written by dev_scanner, refreshed daily at midnight;
                 updater falls back to full TOTP self-auth if expired)
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
#  TOKEN LOADING  (prefer scanner's token, fallback to self-auth)
# ═══════════════════════════════════════════════════════════════════════════════
...
def load_fyers_session(fyers: Fyers, force: bool = False) -> bool:
    ...
    token simultaneously, a random jitter prevents them all from
    racing to TOTP-auth at the same instant (Fyers single-session
    policy would invalidate each other's tokens).
    ...
    log_strategy_event(
        ...
        details=f"Token expired — waiting {jitter:.1f}s before self-auth (anti-collision)",
    )
    sleep(jitter)
...
def verify_and_refresh_token(fyers: Fyers) -> bool:
    ...
    Returns True if token is valid/refreshed, False if all attempts fail.
    """
    # Quick profile check to confirm token is alive
    try:
        resp = fyers.api.get_profile()
        if resp.get("s") == "ok":
            return True
    except Exception:
        pass
...
def main():
    ...
    while True:
        ...
        # ── Step 1c: Periodic token health check (catch mid-day expiry) ────
        if not verify_and_refresh_token(fyers):
            session_ok = False
            write_app_status(
                mode, str(current_day), status="reauth",
                message="Token expired mid-session — re-authenticating...",
            )
            sleep(5)
            continue
        ...
        for symbol_key, info in com_pairs.items():
            ...
            if ok:
                ...
                # Auth error during processing? Try re-auth and retry once
                if not verify_and_refresh_token(fyers):
                    session_ok = False
                    log_strategy_event(
                        symbol_key, "AUTH", "REAUTH_DURING_PROCESS",
                        details="Token died during symbol processing — will re-auth next cycle",
                    )
                    break
